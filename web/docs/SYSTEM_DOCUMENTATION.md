...
```
SchoolSystemUganda/
├── backend/                    # Express Backend Server (Modular Monolith)
│   └── src/
│       ├── models/             # Shared Mongoose Schemas (Student, Class, Grade, Fee...)
│       ├── routes/             # Primary REST API Routes
│       └── modules/            # 12 Domain Modules (finance, hr, hostel, admissions...)
├── mobile/                     # React Native Expo Mobile App
│   ├── assets/                 # App icons, splash screens
│   └── src/
│       ├── screens/            # TeacherPortal, SupervisorPortal, ParentPortal, StudentPortal
│       ├── services/           # Mobile API Client & Offline Cache
│       └── types/              # Domain TypeScript Types
└── web/                        # React Vite Web Console
    ├── docs/                   # Complete System Documentation
    ├── screens/                # Showcase Screenshots & Role Walkthroughs
    └── src/
        ├── components/         # Layout, Navigation & Reusable Components
        ├── context/            # AuthContext
        ├── pages/              # Module Page Views (Admissions, Finance, HR, Hostel, Library...)
...
### 3. Running Locally
```bash
# Run full stack concurrently (Backend + Web)
npm run start

# Or run individual services:
npm run start:backend     # Express API (Port 5000)
npm run start:web         # React Vite Web Console (Port 5173)
