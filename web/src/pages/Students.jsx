...
export default function Students() {
  ...
  const canManageStudents = ['super-admin', 'admin', 'headteacher', 'registrar'].includes(user?.role);
  const canDeleteStudents = ['super-admin', 'admin', 'headteacher'].includes(user?.role);
  ...
  const filteredStudents = students.filter(student => {
    ...
    const matchesClass = classFilter ? String(student.currentClass?._id) === classFilter : true;
        ...
        <table style={styles.table}>
          <thead>
            <tr style={styles.tableHeaderRow}>
              ...
              {canManageStudents && <th style={styles.th}>Actions</th>}
            </tr>
          </thead>
          <tbody>
            {filteredStudents.length > 0 ? (
                  ...
                  {canManageStudents && (
                    <td style={styles.td}>
                      <div style={styles.btnGroup}>
                        <button style={styles.editBtn} aria-label={`Edit ${student.user?.name}`}>
                          <Edit3 size={15} />
                        </button>
                      {canDeleteStudents && <button onClick={() => handleDelete(student._id)} style={styles.deleteBtn} aria-label={`Delete ${student.user?.name}`}>
                        <Trash2 size={15} />
                      </button>}
                      </div>
                    </td>
              ...
              <tr>
                <td colSpan={canManageStudents ? 7 : 6} style={styles.noData}>
