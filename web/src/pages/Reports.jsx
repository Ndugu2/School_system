...
export default function Reports() {
  ...
  const defaultReport = role === 'admin' || role === 'super-admin' ? 'overview' : 'academic';
  ...
  const visibleReportIds = {
    admin: ['overview', 'academic', 'attendance', 'finance', 'staff', 'earlywarning'],
    'super-admin': ['overview', 'academic', 'attendance', 'finance', 'staff', 'earlywarning'],
    teacher: ['academic', 'attendance'],
    student: ['academic', 'attendance', 'finance'],
    parent: ['academic', 'attendance', 'finance'],
  }[role] || ['academic', 'attendance'];
  ...
  useEffect(() => {
    if (!visibleReportIds.includes(activeReport)) {
      setActiveReport(defaultReport);
    }
    if (activeReport === 'earlywarning' && visibleReportIds.includes('earlywarning')) fetchWatchlist();
  }, [activeReport, defaultReport, fetchWatchlist, visibleReportIds]);
  ...
  ].filter(category => visibleReportIds.includes(category.id));
    ...
    <div style={styles.container}>
      ...
      <div style={styles.pageHeader}>
        <div>
          ...
          <p style={styles.pageSubtitle}>{role === 'admin' || role === 'super-admin' ? 'Institutional reports' : 'Reports available to your role'} for {selectedTerm}</p>
        </div>
      ...
      {activeReport === 'earlywarning' && (
        <div style={styles.reportBody}>
          <div style={styles.panel}>
            <div style={styles.panelHeader}>
              <ShieldAlert size={18} color="#ef4444" />
              <h3 style={styles.panelTitle}>Early Warning & Student Risk Watchlist</h3>
            </div>

            <div style={styles.toolbarRow}>
              <button onClick={handleCalculateRisk} style={styles.primaryAction} disabled={calculating}>
                {calculating ? <Loader size={16} style={{ animation: 'spin 1s linear infinite' }} /> : <RefreshCw size={16} />}
                {calculating ? 'Calculating...' : 'Calculate New Risk Report'}
              </button>
            </div>

            {watchlistError && <div style={styles.errorBox}>{watchlistError}</div>}

            {watchlistLoading ? (
              <div style={styles.emptyState}>Loading watchlist...</div>
            ) : watchlist.length === 0 ? (
              <div style={styles.emptyState}>No students currently flagged for academic risk.</div>
            ) : (
              <div style={{ overflowX: 'auto' }}>
                <table style={styles.table}>
                  <thead>
                  ...
                  <tbody>
                    {watchlist.map((student, index) => {
                      const riskLevel = student.riskScore >= 75 ? 'Critical' : student.riskScore >= 50 ? 'Moderate' : 'Low';
                      const badgeStyle = student.riskScore >= 75 ? styles.badgeDanger : student.riskScore >= 50 ? styles.badgeWarn : styles.badgeGood;
                      return (
                        <tr key={student._id || index} style={index % 2 === 0 ? styles.trEven : styles.trOdd}>
                          <td style={{ ...styles.td, fontWeight: 700 }}>{student.studentName || student.name || 'Unknown Student'}</td>
                          <td style={styles.td}>{student.className || student.class || 'N/A'}</td>
                          <td style={{ ...styles.td, fontWeight: 800, color: '#ef4444' }}>{student.riskScore ?? 0}%</td>
                          <td style={styles.td}><span style={badgeStyle}>{riskLevel}</span></td>
                          <td style={styles.td}>{student.attendanceRate ?? 0}%</td>
                          <td style={styles.td}>
                            <button
                              style={styles.emailBtn}
                              onClick={() => handleSendEmail(student.studentId || student._id)}
                              disabled={emailSending === (student.studentId || student._id)}
                            >
                              {emailSending === (student.studentId || student._id) ? <Loader size={14} style={{ animation: 'spin 1s linear infinite' }} /> : <Mail size={14} />}
                              {emailSending === (student.studentId || student._id) ? 'Sending...' : 'Send Alert'}
                            </button>
                            {emailSuccess === (student.studentId || student._id) && <span style={styles.successPill}>Sent</span>}
                          </td>
                        </tr>
                      );
                    })}
...
const styles = {
  ...
  toolbarRow: { display: 'flex', justifyContent: 'flex-end', marginBottom: 16 },
  primaryAction: {
  ...
  successPill: {
    ...
    padding: '3px 8px', borderRadius: 999, backgroundColor: '#d1fae5',
    color: '#065f46', fontSize: 11, fontWeight: 700,
  },
  emptyState: {
    padding: '36px 16px', textAlign: 'center', color: 'var(--text-tertiary)',
    fontSize: 14,
  },
  errorBox: {
    backgroundColor: '#fee2e2', color: '#991b1b', border: '1px solid #fecaca',
    borderRadius: 8, padding: '12px 14px', fontSize: 13, fontWeight: 600, marginBottom: 16,
  },
};
