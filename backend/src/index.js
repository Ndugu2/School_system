...
// Load a server-local .env when present, then fall back to the workspace .env.
// This keeps `npm start` working from the root workspace without overriding
...
app.use(cors({
  origin(origin, callback) {
    // Requests without an Origin header include local tools and same-server calls.
...
if (!process.env.MONGODB_URI) {
  console.error('MONGODB_URI is not configured. Add it to .env before starting the API.');
  process.exit(1);
}

if (!process.env.JWT_SECRET || !process.env.JWT_REFRESH_SECRET) {
  console.error('JWT_SECRET and JWT_REFRESH_SECRET must be configured before starting the API.');
  process.exit(1);
}
