...
// @desc  Manually send risk alert email for a specific student
// @route POST /api/analytics/send-risk-email
router.post('/send-risk-email', protect, authorize('super-admin', 'admin'), async (req, res) => {
  ...
  try {
    const profile = await RiskProfile.findOne({ student: studentId });
    if (!profile) {
      return res.status(404).json({ error: { message: 'No risk profile found for this student' } });
    }
    ...
    const parentObj = student.parentUser
      ? { name: student.parentName, email: student.parentUser.email }
      : { name: student.parentName, email: student.parentEmail };

    if (!parentObj.email) {
      return res.status(400).json({ error: { message: 'No parent email on record for this student' } });
    }

    const studentObj = { name: student.user?.name || 'Student' };

    await sendCriticalRiskEmail(parentObj, studentObj, profile);
    res.json({ message: `Risk alert email sent to ${parentObj.email}` });
  } catch (err) {
    res.status(500).json({ error: { message: err.message } });
  }
});
