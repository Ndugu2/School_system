...
import {
  ...
  FileText, Users, CheckCircle, BarChart2, RefreshCw, CalendarDays, ArrowUpRight
} from 'lucide-react';
import {
...
export default function FinanceDashboard({ setActiveFinanceTab }) {
  ...
  const loadDashboard = useCallback(async () => {
    ...
    try {
      ...
      setError('Financial data is unavailable right now. Check the connection and try again.');
    } finally {
      setLoading(false);
    }
  }, [year]);
  ...
  if (loading) return <div className="page-loading" style={s.loading}><span className="loading-dot" />Loading financial workspace...</div>;
  ...
  return (
    <div className="finance-dashboard" style={s.container}>
      <section className="dashboard-heading" style={s.heading}>
        <div>
          <div className="eyebrow"><CalendarDays size={14} /> Academic year {year}</div>
          <h1 style={s.title}>Finance</h1>
          <p style={s.subtitle}>Collections, expenditure, and outstanding actions for the current academic year.</p>
        </div>
        <div className="dashboard-heading-actions">
          {updatedAt && <span className="updated-label">Updated {updatedAt.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>}
          <button className="secondary-button" onClick={loadDashboard}><RefreshCw size={16} /> Refresh</button>
        </div>
      </section>

      {error && <div className="inline-alert" role="status"><AlertCircle size={18} /><span>{error}</span><button onClick={loadDashboard}>Try again</button></div>}
      {/* KPI Cards */}
      <div style={s.kpiGrid}>
        {revenueCards.map(card => {
          const Icon = card.icon;
          return (
            <div key={card.label} className="metric-card" style={s.kpiCard}>
      ...
      <div style={s.chartsRow}>
        ...
        <div className="surface-card" style={s.chartCard}>
        ...
        <div className="surface-card" style={{ ...s.chartCard, gridColumn: '1 / -1' }}>
          <h3 style={s.chartTitle}>Cash-flow forecast</h3>
          {cashflow?.forecast?.length > 0 ? (
      ...
      <div style={s.quickActions}>
        {[
          { label: 'Generate invoices', icon: FileText, tab: 'invoices', desc: 'Create term invoices by class' },
          { label: 'Record a payment', icon: CheckCircle, tab: 'invoices', desc: 'Apply a payment to an invoice' },
          { label: 'Process payroll', icon: Users, tab: 'payroll', desc: 'Prepare the monthly payroll run' },
          { label: 'Add an expense', icon: AlertCircle, tab: 'expenses', desc: 'Record an operational expense' },
        ].map(action => {
          const Icon = action.icon;
          return (
            <button key={action.label} className="action-tile" style={s.actionTile} onClick={() => setActiveFinanceTab(action.tab)}>
...
const s = {
  ...
  heading: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', gap: 24, padding: '4px 0 6px' },
  title: { fontSize: 28, lineHeight: 1.15, letterSpacing: '-0.8px', color: 'var(--text-primary)', marginTop: 6 },
  subtitle: { maxWidth: 600, color: 'var(--text-secondary)', fontSize: 14, marginTop: 8 },
  loading: { minHeight: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, color: 'var(--text-secondary)', fontWeight: 600 },
